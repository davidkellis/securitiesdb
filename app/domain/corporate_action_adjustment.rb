module CorporateActionAdjustment
  class << self
    ...
    # adjustment_base_date is the date (occurring on or after observation_date) from which our adjustment "look back" is made
    #
    ...
    def calculate_cumulative_adjustment_factor(security, observation_date, adjustment_base_date)
      # 1. look up all corporate actions between (observation_date, adjustment_base_date]
      corporate_actions = CorporateAction.
        where(
          security_id: security.id
          corporate_action_type_id: [CorporateActionType.cash_dividend.id, CorporateActionType.split.id]
        ).
        where {
          (ex_date > observation_date) &
          (ex_date <= adjustment_base_date)
        }
        to_a
